    ...
    <tr>
       ...
       <?php } ?>
        ...
        <th> <?php if($row["daten"]==null){ 
               ?><a href="editaddhour.php?update_number=<?php echo $row["numberhour"]; ?>&nameback=<?php echo "addhour2.php" ?>" class="btn btn-warning">แก้ไข</a><?php
            }else if($row["daten"]!=null){
                echo $alert="ชดเชยไปแล้วไม่สามารถแก้ไขได้";
            }
            ?></th>
        <th> <?php if($row["daten"]==null){ 
               ?><a href="createcomp1.php?check_hour=<?php echo $row["numberhour"];?>&nameback=<?php echo "addhour2.php" ?>" class="btn btn-warning">สอนชดเชย</a><?php
            }else if($row["daten"]!=null){
...
?>
        ...
        <?php if($a == 1){ ?>
                ...
                <?php $d=strtotime("+$w1 week",$startdate ); ?>
                      <th><?php echo $c  ?></th>
                      <form action="testaddhour1_db.php?check_id=<?php echo $c ?>&nameback=<?php echo "addhour2.php" ?>&check_date=<?php echo date("Y-m-d", $d)?>&check_time1=<?php echo date("H:i", strtotime($resultarray['time1']));?>&check_time11=<?php echo date("H:i", strtotime($resultarray['time11']));?>" method="post">
        ...
        <th><?php echo $c  ?></th>
        <form action="testaddhour1_db.php?check_id=<?php echo $c ?>&check_date=<?php echo date("Y-m-d", $d2) ?>&nameback=<?php echo "addhour2.php" ?>&check_time1=<?php echo date("H:i", strtotime($resultarray['time2']));?>&check_time11=<?php echo date("H:i", strtotime($resultarray['time22']));?>" method="post">
