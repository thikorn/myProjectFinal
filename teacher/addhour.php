...
<head>
    ...
    <title>บันทึกการสอนบรรยาย</title>
...
<table>
   ...
   <tr>
    ...
    <tr>
        ...
        <th><?php echo date("H:i", strtotime($row['time1']));?></th>
        <th><?php echo date("H:i", strtotime($row['time11']));?></th>
        ...
        <th> <?php if($row["daten"]==null){ 
               ?><a href="editaddhour.php?update_number=<?php echo $row["numberhour"]; ?>" class="btn btn-warning">แก้ไข</a><?php
            }else if($row["daten"]!=null){
                echo $alert="ชดเชยไปแล้วไม่สามารถแก้ไขได้";
            }
            ?></th>
        <th> <?php if($row["daten"]==null){ 
               ?><a href="createcomp1.php?check_hour=<?php echo $row["numberhour"];?>" class="btn btn-warning">สอนชดเชย</a><?php
            }else if($row["daten"]!=null){
        ...
        <?php
        $stmt = $db->prepare("SELECT * from teaching_information WHERE ids = $ids");
        $stmt->execute();
        ...
        <form action="addhour1_db.php?check_id=<?php echo $i+1 ?>" method="post">
        <th> <?php echo $i+1 ?> </th>
