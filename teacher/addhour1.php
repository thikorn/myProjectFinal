    ...
    <tr>
       ...
       <?php } ?>
        ...
        <th> <?php if($row["daten"]==null){ 
               ?><a href="editaddhour.php?update_number=<?php echo $row["numberhour"]; ?>&nameback=<?php echo "addhour1.php" ?>" class="btn btn-warning">แก้ไข</a><?php
            }else if($row["daten"]!=null){
                echo $alert="ชดเชยไปแล้วไม่สามารถแก้ไขได้";
            }
            ?></th>
        <th> <?php if($row["daten"]==null){ 
               ?><a href="createcomp1.php?check_hour=<?php echo $row["numberhour"];?>&nameback=<?php echo "addhour1.php" ?>" class="btn btn-warning">สอนชดเชย</a><?php
            }else if($row["daten"]!=null){
        ...
        <?php for($c=$numbern;$c<=15;$c++){ ?>
            ...
            <?php if($c == 15){ $w1 = 14;} ?>
                ...
                <?php $d=strtotime("+$w1 week",$startdate ); ?>
                      <th><?php echo $c  ?></th>
                      <form action="testaddhour1_db.php?check_id=<?php echo $c ?>&nameback=<?php echo "addhour1.php" ?>&check_date=<?php echo date("Y-m-d", $d)?>&check_time1=<?php echo date("H:i", strtotime($resultarray['time1']));?>&check_time11=<?php echo date("H:i", strtotime($resultarray['time11']));?>" method="post">
