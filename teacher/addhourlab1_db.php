<?php
    ...
    if (isset($_POST['btn_addhour']) AND (isset($_REQUEST['check_id'])) ) {
        ...
        try{
            $select_stmt = $db->prepare("SELECT topiclab FROM teaching_informationlab WHERE topiclab = :utopic ");
            $select_stmt->bindParam(":utopic", $topic);
            $select_stmt->execute();
            $row = $select_stmt->fetch(PDO::FETCH_ASSOC);
            if($timeto1 < 1){
                $_SESSION['error'] = "เวลาเรียนไม่ถึง  1 ชั่วโมง กรอกใหม่อีกครั้ง";
                header("location:addhourlab.php");
